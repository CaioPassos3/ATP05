...
def tansform_raw_dataset(dataset_file_path: str=None) -> None:
  # Carregar CSV com separador ;
    ...
    # (1) Remover arquivos irrelevantes (e.g., com "test", "example", "doc" no nome)
    df = df[~df['FILE'].str.contains(r'test|example|doc|setup|benchmark', case=False)]

    # (2) Remover linhas com métricas todas zeradas (exceto 'BUG' e 'FILE')
    metric_cols = ['LOC', 'COM', 'BLK', 'NOF', 'NOC', 'APF', 'AMC', 'NER', 'NEH', 'CYC', 'MAD']
    df = df[~(df[metric_cols].sum(axis=1) == 0)]

    # (3) Remover outliers (linhas com valores acima do limite superior para qualquer métrica)
...
def start():
    token = "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"
